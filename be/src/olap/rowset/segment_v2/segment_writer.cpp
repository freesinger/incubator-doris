...
Status SegmentWriter::init(uint32_t write_mbytes_per_sec __attribute__((unused))) {
    ...
    for (auto& column : _tablet_schema->columns()) {
        ...
        // and not support zone map for array type and json type.
        ...
        if (column.type() == FieldType::OLAP_FIELD_TYPE_JSON) {
            opts.need_zone_map = false;
            if (opts.need_bloom_filter) {
                return Status::NotSupported("Do not support bloom filter for json type");
            }
            if (opts.need_bitmap_index) {
                return Status::NotSupported("Do not support bitmap index for json type");
            }
        }
